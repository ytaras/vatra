angular.module("Vatra.controllers.Main",
  ["Vatra.services.Data", "Vatra.model.userInput"]).controller "MainController", ($scope, calculateSights, targetProperties) ->
    $scope.types = [
    ...
    $scope.count = (data) ->
      # TODO: Make binding
      $scope.model = targetProperties()

      $scope.model.type = data.type
      $scope.model.trajectory = data.trajectory
      $scope.model.task = data.task
      $scope.model.distance = data.distance
      $scope.validate()
